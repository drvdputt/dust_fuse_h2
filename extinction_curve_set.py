...
class ExtinctionCurveSet:
    ...
    def w_to_x(w):
        """
        Convert wavelength in angstrom to x value used in FM90 function
        """
        return 1 / (w / ExtinctionCurveSet.micron)

    @staticmethod
    def F(x):
        """
        The shape of the C4 term
        """
    ...
    def D(self, x):
        ...
        The drude function
        """
        return x**2 / ((x**2 - self.x0**2) ** 2 + (x * self.gamma) ** 2)
    ...
    def evaluate(self, w):
        ...
        # drude
        Aw_AV = (
            self.cav1 + self.cav2 * x + self.cav3 * self.D(x) + self.cav4 * self.F(x)
        )
        return Aw_AV
