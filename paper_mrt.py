...
def convert_gastable_to_mrt(data, fn):
    ...
    q_unit = [u.dex(u.cm**-2), u.dex(u.cm**-2), u.dex(u.cm**-2), u.kpc]
    q_descriptions = [
    ...
    for q, description, unit in zip(q_with_unc, q_descriptions, q_unit):
        q_unc = q + "_unc"
        new_colname = q
        new_colname_unc = "e_" + new_colname

        gasmrt[new_colname] = data[q] * unit
        gasmrt[new_colname].description = description
        gasmrt[new_colname_unc] = data[q_unc] * unit
    ...
    q_other_unit = [None, u.cm**-3, None]
    for q, description, unit in zip(q_other, q_other_descriptions, q_other_unit):
        new_colname = q

        gasmrt[new_colname] = data[q] * unit if unit is not None else data[q]
        gasmrt[new_colname].description = description
...
def convert_h2table_to_mrt(h2data, fn):
    ...
    for j in range(8):
        ...
        # do not multiply by unit here! That breaks MaskedColumn
        # (changes masked into nan)
        h2mrt[new_colname].unit = u.dex(u.cm**-2)
        h2mrt[new_colname].description = f"Log10 of column density H2(J = {j})"
        h2mrt[new_uncname] = h2data[old_uncname]
        h2mrt[new_uncname].unit = u.dex(u.cm**-2)
